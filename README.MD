...
4. **Configure settings**:
   ...
   - **Run Attribution Overview**: Optional portfolio-level overview workflow
   - **Citation Preferences**: Set Require Citations, Prioritize Sources, and preferred domains inline
5. **Customize contribution settings** (optional): Use **Contribution Settings** for prompt/model controls
6. **Configure attribution settings** (optional): Use **Attribution Settings** for a separate attribution prompt/model
7. **Click "Generate Commentary"** to start

## Output

The application generates:
- **Excel workbook**: One sheet per portfolio with commentary and sources
- **Optional overview row**: When enabled, a portfolio-level `overview` row appears above the security table
...
Access via the **Contribution Settings** button → **Thinking Level** dropdown.
...
Access via the **Contribution Settings** button → **Text Verbosity** dropdown.

### Dual Prompt System

The application supports two types of prompts:
- **User Prompt**: The main template with variables (`{ticker}`, `{security_name}`, `{period}`, `{source_instructions}`)
- **System Prompt**: Developer-level instructions that guide the AI's behavior

Both are editable in the **Contribution Settings** modal.

### Attribution Settings (Optional)

When **Run Attribution Overview** is enabled, the app runs a separate portfolio-level prompt before writing output.

- Uses attribution tabs from the same input workbook:
  - `AttributionbySector` (expected)
  - `AttributionbyCountryMasterRisk` (optional)
- Uses separate prompt/model/reasoning/verbosity settings in the **Attribution Settings** modal
- Injects parsed top-level attribution data into attribution prompt variables:
  - `{sector_attrib}`
  - `{country_attrib}`
- Writes one `overview` row above the security-level table in each portfolio sheet.
...
- contribution prompts/settings
- attribution workflow prompts/settings
- run-attribution toggle
- preferred sources
- last output folder
...
Optional attribution sheets for overview workflow:
- `AttributionbySector` (expected for attribution workflow)
- `AttributionbyCountryMasterRisk` (optional)

Attribution parsing keeps only the highest-level grouped rows (minimum outline level present in the sheet) plus the `Total` row.
Attribution prompt injection uses one markdown table per sheet with `Total` as the final row when present.

## Documentation

| Document | Description |
|----------|-------------|
| [docs/DEVELOPER_GUIDE.md](docs/DEVELOPER_GUIDE.md) | Architecture, module reference, contributing |
| [docs/CONFIGURATION.md](docs/CONFIGURATION.md) | Config file schema and options |
| [docs/GUI_GUIDE.md](docs/GUI_GUIDE.md) | User interface walkthrough |
| [docs/BUILD_ONEFILE.md](docs/BUILD_ONEFILE.md) | Build unsigned onefile macOS/Windows artifacts |
| [Process_Outline.md](Process_Outline.md) | Full workflow specification |
| [agents.md](agents.md) | AI coding agent quick reference |
